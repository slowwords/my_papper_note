...
​	在本文中，我们使用扩散模型[38]作为生成方法。与其他生成模型一样，DDPM学习给定训练集的图像分布。推理过程通过对随机噪声矢量$x_T$进行采样并逐渐对其进行去噪，直到其达到高质量输出图像$x_0$。在训练期间，DDPM方法定义了一个扩散过程，将图像$x_0$在T个时间步长中转换为高斯白噪声$x_T∼ {\cal N}(0,1)$。forward方向上的每一步由下式给出， 
$$
